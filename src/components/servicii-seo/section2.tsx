...
const Section2 = (props : {
    children: ReactNode;lead: string[]; heading:string; paragrafe: string[]
}) => {
  return (
    <section className={tw(`overflow-hidden`)}>
      <div className={tw('flex flex-col m-14')}>
        {props.lead.map((p: string) => {
        ...
        <p className={tw('my-3 text-lg block m-auto md:text-xl tracking-wide ext-xl font-semibold')}>Nu asta îți dorești, nu-i așa?</p>

        {props.children}

        <p className={tw('my-3 text-lg  md:text-xl tracking-wide')}>
          Hai să spunem lucrurilor pe nume! Se poate și altfel! Doar că implică mai multă muncă. Da, este mult de muncă,
          investiții pentru câteva luni…{' '}
          <span>
